...
class TransfuserFeatureBuilder(AbstractFeatureBuilder):
    ...
    _SAVE_ROOT = Path("/data/goodhsm2000/repos/InternVL/VLM_input")
    _SAVE_ROOT_HD = Path("/data/goodhsm2000/repos/InternVL/HD_Map")
    ...
    def __init__(self, config: TransfuserConfig):
        ...
        self._SAVE_ROOT_HD.mkdir(parents=True, exist_ok=True)
        self._saved_tokens: set[str] = set()

    def get_unique_name(self) -> str:
        """Inherited, see superclass."""
        return "transfuser_feature"

    def compute_features(self, agent_input: AgentInput, token: Optional[str] = None,) -> Dict[str, torch.Tensor]:
        """Inherited, see superclass."""
        features = {}

        features["camera_feature"] = self._get_camera_feature(agent_input, token)
    ...
    def _get_camera_feature(self, agent_input: AgentInput, token: Optional[str] = None,) -> torch.Tensor:
        """
        ...
        """
        token이름대로 img 저장 부분분
        """
        # if token is not None and token not in self._saved_tokens:
        #     save_name = f"{token}.jpg"
        #     cv2.imwrite(str(self._SAVE_ROOT / save_name), resized_image, [cv2.IMWRITE_JPEG_QUALITY, 95])
        
        # self._saved_tokens.add(token)
...
class TransfuserTargetBuilder(AbstractTargetBuilder):
    ...
    def compute_targets(self, scene: Scene, token: Optional[str] = None,) -> Dict[str, torch.Tensor]:
        """Inherited, see superclass."""
        ...
        base_colors = [
            (  0,   0,   0),    # 0 background
            (160, 160, 160),    # 1 road
            (255, 255, 255),    # 2 walkway
            ( 50, 200, 255),    # 3 centerline
            (  0, 128, 255),    # 4 static (orange)
            (255, 255, 255),    # 5 vehicle
            (  0,   0, 255),    # 6 pedestrian
        ]

        # 팔레트 길이 그대로 LUT 생성  (행 = 7, 열 = 3)
        lut = np.array(base_colors, dtype=np.uint8)

        # 만약 bev_semantic_map 에 팔레트 범위 밖 라벨이 들어오면 흰색으로 표시
        extra_rows = int(bev_semantic_map.max()) + 1 - lut.shape[0]
        if extra_rows > 0:
            lut = np.vstack([lut, np.ones((extra_rows, 3), np.uint8) * 255])

        # ── 라벨 → RGB ──────────────────────────────
        rgb = lut[bev_semantic_map.numpy().astype(np.uint8)]          # (H,W,3)

        # 2) 로컬 (0,0) 을 픽셀(u,v)로 바꿔서 삼각형을 그림
        # 이상하게 x, y축이 뒤바껴있음 아마 아래 코드에서 rot90 때문인걸로 예상됨 
        # 즉 세로가 x축, 가로가 y축임
        u, v = self._coords_to_pixel( np.array([[[0, 0]]], dtype=np.float32) )[0,0]

        warnings.warn(f"pixel = {u,v}") # -> (0,128)이 나옴 
                                        
        ego_tri = np.array([[v, u+6], [v-4, u-6], [v+4, u-6]], np.int32)
        cv2.fillPoly(rgb, [ego_tri], (0, 255, 0))
        ...
        if token:
            rgb_save = rgb[::-1, ::-1]
            save_dir = Path("/data/goodhsm2000/repos/InternVL/HD_Map")
            cv2.imwrite(str(save_dir / f"{token}_hd.png"), rgb_save)

        # ──────────────────────────────────
        # 4. float HD-map feature (G 채널)
        # ──────────────────────────────────
        gray_float  = (rgb[..., 1] / 255.).astype(np.float32)   # 0-1
        hd_map_feature = torch.from_numpy(gray_float)[None]     # (1,H,W)
