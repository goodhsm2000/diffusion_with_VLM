...
class AbstractAgent(torch.nn.Module, ABC):
    ...
    def forward(self, features: Dict[str, torch.Tensor], token: str) -> Dict[str, torch.Tensor]:
        """
    ...
    def compute_trajectory(self, agent_input: AgentInput, token: str, scene = None) -> Trajectory:
        """
        ...
        # forward pass
        with torch.no_grad():
            predictions = self.forward(features, token)
