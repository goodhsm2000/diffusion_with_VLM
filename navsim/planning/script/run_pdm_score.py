...
def run_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[Dict[str, Any]]:
    ...
    saveimg = agent.get_feature_builders()
    ...
    for idx, token in enumerate(tokens_to_evaluate):
        ...
        try:
            ...
            # -------------------------------------------
            #  여기부터 compute_trajectory 호출 시간 측정
            start_time = time.time()
            if agent.requires_scene:
                scene = scene_loader.get_scene_from_token(token)
                trajectory = agent.compute_trajectory(agent_input, token, scene)
            else:
    ...
    # 모든 시나리오 처리가 끝난 뒤, 평균 inference 시간 및 FPS 계산
    if inference_times:
        avg_time = sum(inference_times) / len(inference_times)
        fps = 1.0 / avg_time if avg_time > 0 else float("inf")
        logger.info(
            f"[Thread {thread_id}, Node {node_id}] "
            f"Average inference time per token: {avg_time:.4f} sec, "
            f"Approx. FPS (per-token): {fps:.3f}"
        )
