...
class Dataset(torch.utils.data.Dataset):
    ...
    def _cache_scene_with_token(self, token: str) -> None:
        ...
        for builder in self._feature_builders:
            data_dict_path = token_path / (builder.get_unique_name() + ".gz")
            data_dict = builder.compute_features(agent_input, token=metadata.initial_token)
            dump_feature_target_to_pickle(data_dict_path, data_dict)

        for builder in self._target_builders:
            data_dict_path = token_path / (builder.get_unique_name() + ".gz")
            data_dict = builder.compute_targets(scene, token=metadata.initial_token)
            dump_feature_target_to_pickle(data_dict_path, data_dict)
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor]]:
        ...
        if self._cache_path is not None:
            ...
            agent_input = scene.get_agent_input()
            for builder in self._feature_builders:
                features.update(builder.compute_features(agent_input, token))
            for builder in self._target_builders:
                targets.update(builder.compute_targets(scene, token))
